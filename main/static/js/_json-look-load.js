...
async function sendData(data) {
    ...
    let url = `/looks?${arr[0][0]}=${arr[0][1]}&${arr[1][0]}=${arr[1][1]} `;
    // console.log(url);
    ...
    function createColorTable(json) {
        ...
        for (el in json) {
            uniqCollection.add(json[el]['showplacesType']);
        };
        let i = 0
...
function createCards(data, colorTable) {
    ...
    card.insertAdjacentHTML('afterbegin',
        `<div class="hotels-card-elem">
            ...
            <div class="hotels-card-elem__type ${colorTable[data['showplacesType']]}">
                <p>${data['showplacesType']}</p>
            </div>
